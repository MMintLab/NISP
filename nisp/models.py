...
from nisp import archs
from nisp.utils import flatten_pytree
...
class TrainState(train_state.TrainState):
    ...
    def apply_weights(self, weights, **kwargs):
        ...
        def update_weights(old_weights, new_weights, momentum):
            return tree_map(
                lambda old_w, new_w: running_average(old_w, new_w, momentum),
                old_weights,
                new_weights,
            )
...
class PINN:
    ...
    def compute_weights(self, params, batch, *args):
        if self.config.weighting.scheme == "grad_norm":
            ...
            num_elements = sum(
                len(tree_leaves(grad_norm)) for grad_norm in grad_norm_dict.values()
            )
