...
class VehicleSuspension(ForwardBVP):
    ...
    def get_residual(self, params, t):
        ...
        f1 = self.l * q1_t ** 2 * jnp.cos(q1) * (self.m1  + self.m2) + self.l * self.m2 * q2_t ** 2 * jnp.cos(q2) + f
        f2 = - self.l * (self.g * self.m1 * jnp.cos(q1) + self.g * self.m2*jnp.cos(q1) + self.l*self.m2* q2_t **2 * jnp.sin(q1 - q2))
        f3 = self.l * self.m2 * (-self.g * jnp.cos(q2) + self.l * q1_t**2 * jnp.sin(q1 - q2))
