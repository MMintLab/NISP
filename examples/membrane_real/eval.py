...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    pcd = np.concatenate(
        [x_hat.reshape(-1, 1), y_hat.reshape(-1, 1), z_hat.reshape(-1, 1), color],
        axis=-1,
    )
    ...
    ax.scatter(test_coords[..., 0], test_coords[..., 1], z_hat, s=0.5, c="r")

    ax.scatter(x_hat, y_hat, z_hat, s=0.8)
    ellipse_points = load_ellipse(2 * a, 2 * b, num_points=1000)

    ax.scatter(
        ellipse_points[..., 0], ellipse_points[..., 1], ellipse_points[..., 2], s=2
    )
    ...
    sc = plt.scatter(
        x_hat, y_hat, c=f_pred, vmin=min(f_pred), vmax=max(f_pred), s=7, cmap="viridis"
    )
    plt.colorbar(sc)
    plt.xlim(-0.065, 0.065)
    plt.ylim(-0.065, 0.065)
    plt.title(
        f"Contact force estimate {np.round(cnt_fc, 3)}[N], GT  { round(cnt_fc_gt, 3)}[N], [N/m^2]"
    )
    ...
    mask_gt = np.repeat(etc["contact_mask_r"][..., np.newaxis], 3, axis=2) * 255.0
    mask_gt[..., [0, 2]] = 0
    ax1.imshow(mask_gt)
    ...
    color_thres[mask_thres_idx, 1] = 1.0
    ax1.imshow(
        color_thres.reshape(
            etc["delta_depth"].shape[0], etc["delta_depth"].shape[1], -1
        )
    )
    ...
    ax2.imshow(
        color.reshape(etc["delta_depth"].shape[0], etc["delta_depth"].shape[1], -1)
    )
