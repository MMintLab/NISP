...
from jaxpi.chamfer_distance import chamfer_distance_jit, chamfer_distance_directional_jit
...
class Membrane(ForwardBVP):
    ...
    def get_residual(self, params, x, y):
        ...
        stress_term = lambda params, x, y: Nx(params, x, y) * w_aa(params, x, y) + 2 * Nxy(params, x, y) * w_ab(params, x, y) + Ny (params, x, y)* w_bb(params, x, y)

        w_aabb = jit(grad(grad(w_aa, argnums=2), argnums=2)) #(params, x, y)
        w_aaaa = jit(grad(grad(w_aa, argnums=1), argnums=1))#(params, x, y)
        w_bbbb = jit(grad(grad(w_bb, argnums=1), argnums=2))#(params, x, y)

        r_net_f_without_f = self.D  * (w_aaaa(params, x, y) + w_bbbb(params, x, y) + 2*w_aabb(params, x, y)) - self.P - stress_term(params, x, y)
        # r_net_f_without_f = self.gelu(-r_net_f_without_f)

        # jax.debug.print("🤯 {x} 🤯", x=r_net_f_without_f)
        r_net_output =   r_net_f_without_f +  f(params, x, y)
        # r_net_diff_a = self.D * (grad(w_aaaa, argnums=1)(params, x, y)+ grad(w_bbbb, argnums=1)(params, x, y) + 2*grad(w_aabb, argnums=1)(params, x, y)) - grad(stress_term, argnums=1)(params, x, y) 
        # r_net_diff_b = self.D * (grad(w_aaaa, argnums=2)(params, x, y)+ grad(w_bbbb, argnums=2)(params, x, y) + 2*grad(w_aabb, argnums=2)(params, x, y)) - grad(stress_term, argnums=2)(params, x, y)

        # rp_net_output = (r_net_diff_a + grad(f, argnums=1)(params, x, y))**2 + (r_net_diff_b+ grad(f, argnums=2)(params, x, y) )**2
        # rp_net_output_without_f = r_net_diff_a **2 + r_net_diff_b **2
        return r_net_f_without_f, r_net_output, r_net_f_without_f, r_net_output,
    ...
    def losses(self, params,  batch):
        ...
        u_bc_pred = self.u_pred_fn(
            params, self.coords[:, 0], self.coords[:, 1]
        )
        v_bc_pred = self.v_pred_fn(
            params, self.coords[:, 0], self.coords[:, 1]
        )
        w_bc_pred = self.w_pred_fn(
            params, self.coords[:, 0], self.coords[:, 1]
        )
        f_bc_pred = self.f_pred_fn(
            params, self.coords[:, 0], self.coords[:, 1]
        )       
        bc_pred = jnp.stack([u_bc_pred + self.coords[:, 0],
                              v_bc_pred + self.coords[:, 1], 
                              w_bc_pred], axis = 1)
        ...
        # r_pred_nf = self.r_pred_fn(
        #     params, self.coords[:, 0], self.coords[:, 1]
        # )
        # jax.debug.print("🤯 {x} 🤯", x=r_pred_nf)

        # f_in_pred = - self.r_nf_pred_fn(
        #     params, self.wall_coords[:, 0], self.wall_coords[:, 1]
        # )
        ...
        eps = 0.0
        push_only = jnp.where(f_in_pred < eps, f_in_pred, jnp.zeros_like(f_in_pred) ) # if un smaller than eps, than no contact.
...
class MembraneEvaluator(BaseEvaluator):
    ...
    def log_errors(self, params, coords):
        ...
        # f_pred = self.model.r_nf_pred_fn(params, coords[:, 0], coords[:, 1])
        ...
        pcd = np.concatenate([ (coords[:,0:1] + u_pred.reshape(-1,1)) /self.model.L, (coords[:,1:2] + v_pred.reshape(-1,1)) /self.model.L, 
        ...
        #                     w_pred.reshape(-1,1)/self.model.L , np.zeros(( len(coords),3))], axis = -1)
        # save_pointcloud(pcd, filename=f'pred_train_w.ply', save_path='.')
        # w_error = abs(w_error)
