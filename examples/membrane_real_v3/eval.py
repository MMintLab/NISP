...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    # ax1.scatter(pcd[:,0], pcd[:,1] ,c = color_thres)
    ...
    # estimated_contact_idx = np.where(f_pred > CONTACT_THRES)
    # color_thres = np.zeros((len(y_hat), 3))
    # color_thres[estimated_contact_idx,1] = 1.
    ax2.imshow(color.reshape(etc["delta_depth"].shape[0], 
                                   etc["delta_depth"].shape[1],
                                     -1))
    # ax2.scatter(x_hat, y_hat ,c = color_thres)
    # ax2.set_xlim(-0.01, 0.03)
    # ax2.set_ylim(-0.025, 0.025)
    # breakpoint()
    # ax2.set_title(f"Contact Pressure Thresholding: {CONTACT_THRES}[N/m^2]")
    ...
    # y = jnp.zeros_like(x)
    # # z_hat = model.r_nf_pred_fn(params, x, y)
    # # z_hat = model.f_pred_fn(params, x, y)



    # fig = plt.figure()
    # z_hat_real = f_pred * (L **2) / L_f
    # plt.plot(x/L, z_hat_real )
    # plt.vlines(-0.01, min(z_hat_real), max(z_hat_real), 'r', '--')
    # plt.vlines(0.04, min(z_hat_real), max(z_hat_real), 'r', '--')

    # plt.show()
    # plt.savefig(f"intersection_force.png")
